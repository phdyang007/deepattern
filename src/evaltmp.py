...
def analysis_tmp(case, dim_max = 24, percent = 1): #case: tc1 tc2 tc3 tc4 tc5 tc6
    ...
    tmp_all_length = len(df_test)
    if not percent == 1:
        sampled_indices = np.array(rd.sample(range(tmp_all_length), int(tmp_all_length*percent)))
        df_test = df_test.iloc[sampled_indices]
...
if __name__== "__main__":
    ...
    div_test, _ =analysis_tmp(case, percent = percent)
